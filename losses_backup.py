...
class XTTSLoss(tf.keras.losses.Loss):
    ...
    def _apply_loss_smoothing(self, current_loss: tf.Tensor) -> tf.Tensor:
        ...
        def _no_spike_branch():
            return current_loss

        step_count_int = tf.cast(self.step_count, tf.int32)
    ...
    def get_stability_metrics(self) -> Dict[str, tf.Tensor]:
        ...
        if self.step_count > 0:
            ...
            if self.step_count > 10:
                ...
                metrics["loss_stability_score"] = tf.exp(-loss_variance)
            else:
                metrics["loss_variance"] = tf.zeros([], dtype=tf.float32)
                metrics["loss_stability_score"] = tf.ones([], dtype=tf.float32)

            metrics["step_count"] = tf.cast(self.step_count, tf.float32)
        else:
            zero = tf.zeros([], dtype=tf.float32)
            metrics["running_mel_loss"] = zero
            metrics["running_total_loss"] = zero
            metrics["loss_variance"] = zero
            metrics["loss_stability_score"] = tf.ones([], dtype=tf.float32)
            metrics["step_count"] = tf.cast(self.step_count, tf.float32)
