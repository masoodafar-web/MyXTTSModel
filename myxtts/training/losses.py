...
__all__ = ['XTTSLoss', 'create_stop_targets', 'mel_loss', 'stop_token_loss']
...
class XTTSLoss(tf.keras.losses.Loss):
    ...
    def call(
        ...
        if "mel_target" in y_true and "mel_output" in y_pred:
            ...
            losses["mel_loss"] = _safe("mel_loss", mel_l)
        ...
        self._raw_total_loss_tensor = raw_total_loss
        weighted_contrib["raw_total_loss"] = _safe("w_raw_total_loss", raw_total_loss)
        self._weighted_losses = {k: _safe(k, v) for k, v in weighted_contrib.items()}
        if self.loss_smoothing_factor > 0.0:
            total_loss = self._apply_loss_smoothing(total_loss)

        # Store individual losses for monitoring
        self.losses = {k: _safe(k, v) for k, v in losses.items()}
    ...
    def _apply_loss_smoothing(self, current_loss: tf.Tensor) -> tf.Tensor:
        ...
        finite_history = tf.math.is_finite(self.loss_history)
        cleaned_history = tf.where(finite_history, self.loss_history, tf.zeros_like(self.loss_history))
        self.loss_history.assign(cleaned_history)

        cleaned_running_total = tf.where(
            tf.math.is_finite(self.running_total_loss),
            tf.cast(self.running_total_loss, tf.float32),
            tf.constant(0.0, dtype=tf.float32)
        )
        self.running_total_loss.assign(cleaned_running_total)
        ...
        # Compute both candidates without control flow to keep the graph simple
        recent_avg = tf.reduce_mean(recent_losses)
        recent_avg_safe = tf.where(
            tf.math.is_finite(recent_avg),
            recent_avg,
            current_loss
        )
        denominator = recent_avg_safe + 1e-8
        ...
        no_spike_candidate = current_loss
        use_spike = tf.greater(tf.cast(self.step_count, tf.int32), 10)
        smoothed_loss = tf.where(use_spike, spike_candidate, no_spike_candidate)
    ...
    def get_stability_metrics(self) -> Dict[str, tf.Tensor]:
        ...
        step_f = tf.cast(self.step_count, tf.float32)
        has_steps = tf.greater(self.step_count, 0)
        running_mel = tf.where(has_steps, tf.cast(self.running_mel_loss, tf.float32), tf.zeros([], tf.float32))
        running_total = tf.where(has_steps, tf.cast(self.running_total_loss, tf.float32), tf.zeros([], tf.float32))
        recent_losses = self.loss_history
        loss_mean = tf.reduce_mean(recent_losses)
        loss_variance = tf.reduce_mean(tf.square(recent_losses - loss_mean))
        variance_when_valid = tf.where(tf.greater(self.step_count, 10), loss_variance, tf.zeros([], tf.float32))
        stability_score = tf.where(tf.greater(self.step_count, 10), tf.exp(-loss_variance), tf.ones([], tf.float32))

        metrics["running_mel_loss"] = running_mel
        metrics["running_total_loss"] = running_total
        metrics["loss_variance"] = variance_when_valid
        metrics["loss_stability_score"] = stability_score
        metrics["step_count"] = step_f
